...
int main(int argc, char **argv)
{
	...
	while(1){
		sc.RegWritePos(5, 500, 0, 0, 0);// Пример: Acc=10
		sc.RegWritePos(6, 500, 0, 0, 0);// Пример: Acc=10
		sc.RegWriteAction();
		std::cout<<"pos = "<<500<<std::endl;
		usleep(750*1000);//[(P1-P0)/V]*1000+100
  
		sc.RegWritePos(5, 600, 0, 0, 0);// Пример: Acc=10
		sc.RegWritePos(6, 600, 0, 0, 0);// Пример: Acc=10
		sc.RegWriteAction();
